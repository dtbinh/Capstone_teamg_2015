...
#include "robographers/AprilTag.h" // rosmsg
#include "robographers/AprilTagList.h" // rosmsg
...
public:
  AprilTagNode() : 
    ...
    tag_detector(NULL),
    camera_focal_length_y(658.616638),
    camera_focal_length_x(656.652100),
    tag_size(0.158), // 1 1/8in marker = 0.029m
    show_debug_image(true)
  {
    ...
    tag_list_pub = nh_.advertise<robographers::AprilTagList>("/april_tags", 100);

    // Use a private node handle so that multiple instances of the node can
    // be run simultaneously while using different parameters.
    ros::NodeHandle private_node_handle("~"); 
    private_node_handle.param<double>("focal_length_px", camera_focal_length_x, 656.652100);
    private_node_handle.param<double>("focal_length_py", camera_focal_length_y, 658.616638);
    private_node_handle.param<double>("tag_size_cm", tag_size, 15.8);
  ...
  robographers::AprilTag convert_to_msg(AprilTags::TagDetection& detection, int width, int height) {
  ...
  void processCvImage(cv_bridge::CvImagePtr cv_ptr) 
  {
    ...
    vector<robographers::AprilTag> tag_msgs;
    ...
    if(detections.size() > 0) { // take this out if you want absence notificaiton
      robographers::AprilTagList tag_list;
